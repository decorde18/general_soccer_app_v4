        ...
        {config.form.fields.map((field) => (
          ...
          >
            <FieldInput
              field={{
                ...field, 
                options: field.options ? [...field.options, ...(dynamicOptions[field.key] || [])] : undefined
              }}
              ...
              onAddOption={(val, label) => {
                setDynamicOptions((prev) => ({
                  ...prev,
                  [field.key]: [...(prev[field.key] || []), { value: val, label: label }],
                }));
              }}
            />
          </div>
...
function FieldInput({
  ...
  const isCheckboxOrToggle = field.type === "checkbox" || field.type === "toggle";
    ...
    <div className='flex flex-col gap-1.5'>
      ...
      {isCheckboxOrToggle ? (
        <div className="flex items-center gap-3 pt-1 pb-2">
           {field.type === "toggle" ? (
             <label className="relative inline-flex items-center cursor-pointer">
               <input type="checkbox" className="sr-only peer" checked={value === "true"} onChange={e => onChange(e.target.checked ? "true" : "false")} />
             ...
             <input type="checkbox" checked={value === "true"} onChange={e => onChange(e.target.checked ? "true" : "false")} className="w-4 h-4 text-primary bg-surface border-border rounded focus:ring-primary/20 focus:ring-2 cursor-pointer" />
           )}
           <span className="text-sm font-medium text-text cursor-pointer select-none" onClick={() => onChange(value === "true" ? "false" : "true")}>
              {field.label}
              {field.required && <span className='text-danger ml-1'>*</span>}
           </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          {isCustom ? (
            ...
            <div className="flex-1">
               <Select
                 ...
                 options={field.creatable ? [...field.options, { label: "+ Create New", value: "__CREATE_NEW__" }] : field.options}
                 error={!!error}
                 width="full"
          ...
          {isCustom && (
            <Button variant="outline" type="button" onClick={() => { setIsCustom(false); onChange(""); }} className="px-2 h-10 border-border" title="Cancel">
               <X size={16} />
            </Button>
          )}
          {showNestedForm && field.creatableConfig && field.onCreatableSubmit && (
             <GenericForm
                config={field.creatableConfig}
                onSubmit={async (data) => {
                   try {
                      const res = await field.onCreatableSubmit!(data);
                      const newVal = res?.value || data.name || data[field.creatableConfig!.form.fields[0].key];
