    ...
    const tanstackColumns = useMemo<ColumnDef<T>[]>(() => {
        const cols: ColumnDef<T>[] = columns.map((col) => ({
            ...
            cell: ({ getValue }) => {
                ...
                if (col.type === 'date') {
                    return val ? new Date(val).toLocaleDateString('en-US', { dateStyle: 'medium' }) : '—'
                }
                ...
                <table className="w-full text-sm border-collapse">
                    ...
                    <tbody>
                        {table.getRowModel().rows.length === 0 ? (
                            <tr>
                                <td colSpan={tanstackColumns.length} className="px-4 py-6 text-center text-sm text-muted">
                                    No results found.
                                </td>
                            </tr>
                        ) : (
                ...
                <div className="flex items-center gap-1">
                    ...
                    {Array.from({ length: table.getPageCount() }, (_, i) => (
                        <button
                            ...
                            className={`w-7 h-7 text-xs rounded-md border transition-colors ${i === pagination.pageIndex
