...
export default function LeaguesStructureClient({
  ...
  const filteredTree = useMemo(() => {
    ...
    const checkNodeMatch = (node: TreeNode): boolean => {
      ...
      const nodeTeams = node.type === "node"
        ? teamEnrollmentsRecords.filter(
            (e) => e.leagueNodeId === node.nodeId && e.seasonId === Number(selectedSeasonId)
          )
        : [];
      const teamMatches = nodeTeams.some(
        (t) => t.teamName.toLowerCase().includes(query) || t.clubName.toLowerCase().includes(query)
      );
  ...
  const selectedNodeEnrollments = useMemo(() => {
    ...
    return teamEnrollmentsRecords.filter(
      (e) => e.leagueNodeId === selectedInfo.id && e.seasonId === Number(selectedSeasonId)
    );
  ...
  const availableTeamSeasonOptions = useMemo(() => {
    ...
    return teamSeasonsData.filter((ts) => !enrolledIds.includes(Number(ts.value)));
  ...
  const handleCreateNodeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createLeagueNode({
        ...
        parentId: addParentItem.type === "node" ? String(addParentItem.nodeId) : "",
      ...
      toast({
        title: "Success",
        description: `Sub-node "${nodeFormName}" created successfully`,
        type: "success",
      });
  ...
  const handleDeleteNodeConfirm = async () => {
    ...
    try {
      ...
      toast({
        title: "Deleted",
        description: `Node "${deleteItem.name}" and all its descendants removed.`,
        type: "success",
      });
      ...
      toast({
        title: "Error",
        description: err.message || "Failed to delete node.",
        type: "error",
      });
    }
  };
  ...
  const handleEnrollTeamSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Enrolled",
        description: "Team successfully enrolled in this division.",
        type: "success",
      });
      ...
      toast({
        title: "Error",
        description: err.message || "Failed to enroll team.",
        type: "error",
      });
    } finally {
  ...
  const handleUnenrollTeam = async (enrollmentId: number, teamName: string) => {
    try {
      ...
      toast({
        title: "Removed",
        description: `Team "${teamName}" removed from division.`,
        type: "success",
      });
      ...
      toast({
        title: "Error",
        description: err.message || "Failed to remove team enrollment.",
        type: "error",
      });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-border/80 pb-4">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-text">Leagues & Structure</h1>
          <p className="text-muted text-sm mt-1">
            Build and manage hierarchical league divisions, age groups, and assign team rosters dynamically.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 bg-surface/50 border border-border px-3 py-1.5 rounded-xl">
          ...
          <span className="text-xs font-semibold text-muted">Enrollments Season:</span>
          <select
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-5 bg-surface border border-border/80 rounded-xl p-4 shadow-sm flex flex-col gap-4 max-h-[75vh]">
          <div className="flex items-center gap-2">
            <h3 className="font-bold text-sm text-text uppercase tracking-wider">Hierarchy Explorer</h3>
          ...
          {selectedInfo ? (
            <div className="bg-surface border border-border/80 rounded-xl p-6 shadow-sm space-y-6">
                  ...
                  {selectedInfo.type === "node" && (
                    <>
                      ...
                      <Button
                        ...
                        size="sm"
                        className="flex items-center gap-1.5"
                        onClick={() => handleOpenDeleteNode(selectedInfo.record)}
                      >
                        <Trash2 size={14} />
                        <span>Delete</span>
                      </Button>
              ...
              {selectedInfo.type === "node" && (
                <div className="grid grid-cols-2 gap-4 bg-surface-hover/40 border border-border/50 rounded-xl p-4 text-sm">
                  <div>
                    <span className="text-muted block text-xs">Hierarchy level</span>
                    <span className="font-bold text-text">{selectedInfo.level ?? 0}</span>
                  </div>
                  <div>
                    <span className="text-muted block text-xs">Display order index</span>
                    <span className="font-bold text-text">{selectedInfo.displayOrder ?? 0}</span>
                  </div>
                </div>
              )}

              {/* Sub-nodes list under Leagues/Nodes */}
              {(selectedInfo.type === "league" || selectedInfo.type === "node") && (
                    ...
                    if (childrenNodes.length === 0) {
                      return (
                        <div className="text-center py-6 border border-dashed border-border rounded-xl text-muted text-sm">
                          No sub-nodes created under this node. Click "+ Sub-node" above to build your hierarchy.
                        </div>
                      );
                    }
                    ...
                    return (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {childrenNodes.map((child) => (
                          <div
                            key={child.id}
                            onClick={() => setSelectedItemId(`node-${child.id}`)}
                          ...
                          >
                            ...
                            <ArrowRight size={14} className="text-muted group-hover:text-primary" />
                          </div>
                        ))}
                      </div>
                    );
                ...
                <div className="border-t border-border/60 pt-6 space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xs text-muted">
                      Season: {seasonsData.find((s) => s.value === selectedSeasonId)?.label}
                    </span>
                  </div>

                  {/* Enroll team form */}
                  <form onSubmit={handleEnrollTeamSubmit} className="flex gap-2">
                    <div className="flex-1">
                      <Select
                        value={enrollFormTeamSeasonId}
                        onChange={(e: any) => setEnrollFormTeamSeasonId(e.target.value)}
                  ...
                  {selectedNodeEnrollments.length > 0 ? (
                      ...
                      {selectedNodeEnrollments.map((team) => (
                        ...
                        >
                          ...
                          <Button
                            ...
                            onClick={() => handleUnenrollTeam(team.id, team.teamName)}
                          >
                            Remove
                          </Button>
                        </div>
                    ...
                    <div className="text-center py-8 border border-dashed border-border rounded-xl text-muted text-sm">
                      No teams enrolled in this division for the selected season.
                    </div>
                  )}
                </div>
              )}
                ...
                <div className="space-y-6">
                  <div className="bg-surface-hover/30 border border-border rounded-xl p-4 space-y-3">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="text-muted block text-xs">Club Organization</span>
                        <span className="font-bold text-text">{(selectedInfo.record as any).clubName}</span>
                      </div>
                      <div>
                        <span className="text-muted block text-xs">Roster Team Name</span>
                        <span className="font-bold text-text">{(selectedInfo.record as any).teamName}</span>
                      </div>
                      <div>
                        <span className="text-muted block text-xs">Competition Season</span>
                        <span className="font-bold text-text">{(selectedInfo.record as any).seasonName}</span>
                      </div>
                      <div>
                        <span className="text-muted block text-xs">Assigned Division / Node</span>
                        <span className="font-bold text-text">{(selectedInfo.record as any).leagueNodeName}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-center border-t border-border pt-6">
                    <Button
                      ...
                      onClick={() => handleUnenrollTeam(selectedInfo.id, selectedInfo.teamName!)}
                    >
                      <Trash2 size={16} />
            ...
            <div className="bg-surface border border-border/85 rounded-xl p-8 shadow-sm text-center py-16 flex flex-col items-center justify-center gap-4 text-muted">
              ...
              <div className="space-y-1 max-w-sm">
                <h4 className="font-bold text-lg text-text">Select Hierarchy Node</h4>
                <p className="text-sm">
                  Click a league, conference, or age division in the hierarchy explorer list to edit details, append sub-levels, or enroll team seasons.
                </p>
              </div>
            </div>
          )}
      ...
      >
        <form onSubmit={handleCreateNodeSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-sm font-bold text-text">Hierarchy Level</label>
              <Input
                type="number"
                placeholder="e.g. 1"
                value={nodeFormLevel}
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-bold text-text">Display Order</label>
              <Input
                type="number"
                placeholder="e.g. 10"
                value={nodeFormOrder}
