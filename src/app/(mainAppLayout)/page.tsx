...
export default async function LandingPage({ searchParams }: PageProps) {
  ...
  let filterSubtitle = "All recent results and upcoming games across the organization.";
  ...
  return (
    <main className="min-h-screen bg-background pb-12">
      ...
      <section className="relative overflow-hidden py-12 sm:py-16 text-center border-b border-border/40 bg-surface/30 backdrop-blur-sm">
        ...
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-lg text-muted/90 max-w-2xl mx-auto leading-relaxed">
            Follow your favorite teams, browse league standings, track upcoming schedules, and view complete game results instantly.
          </p>
        </div>
      </section>
      ...
      <section className="px-4 sm:px-6 lg:px-8 -mt-6 relative z-10">
        <LandingSelectors teamSeasons={teamSeasonOptions} leagues={leagueOptions} />
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 mt-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <h3 className="text-lg font-bold text-text flex items-center gap-2 px-1">
              ...
              <span className="text-xs font-normal text-muted bg-surface border border-border px-2 py-0.5 rounded-full ml-auto">
                {upcomingFixtures.length} {upcomingFixtures.length === 1 ? "Game" : "Games"}
              </span>
            </h3>
            ...
            {upcomingFixtures.length === 0 ? (
              <Card variant="outlined" padding="lg" className="text-center py-12">
                ...
                <p className="text-muted font-medium">No upcoming games scheduled.</p>
              </Card>
            ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 text-xs text-muted mb-2">
                          ...
                          {game.status !== "scheduled" && (
                            <>
                              <span>•</span>
                              <span className="text-accent font-semibold uppercase">{game.status.replace("_", " ")}</span>
                            </>
                          )}
                        </div>
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center gap-2.5">
                            <div className="h-6 w-6 rounded bg-primary/10 border border-primary/20 flex items-center justify-center font-bold text-xs text-primary">H</div>
                            <span className="font-semibold text-text text-sm sm:text-base truncate">
                              {game.homeClubName} {game.homeTeamName}
                            </span>
                          </div>
                          <div className="flex items-center gap-2.5">
                            <div className="h-6 w-6 rounded bg-accent/10 border border-accent/20 flex items-center justify-center font-bold text-xs text-accent">A</div>
                            <span className="font-semibold text-text text-sm sm:text-base truncate">
                      ...
                      <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center border-t sm:border-t-0 sm:border-l border-border/50 pt-3 sm:pt-0 sm:pl-6 gap-2 text-right">
                        ...
                        {game.locationName && (
                          <div className="flex items-center gap-1.5 text-xs text-muted max-w-[150px] truncate">
                            <MapPin size={14} />
                            <span title={game.locationName}>{game.locationName}</span>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <div className="space-y-6">
            <h3 className="text-lg font-bold text-text flex items-center gap-2 px-1">
              ...
              <span className="text-xs font-normal text-muted bg-surface border border-border px-2 py-0.5 rounded-full ml-auto">
                {recentResults.length} {recentResults.length === 1 ? "Result" : "Results"}
              </span>
            </h3>
            ...
            {recentResults.length === 0 ? (
              <Card variant="outlined" padding="lg" className="text-center py-12">
                <ShieldAlert size={40} className="mx-auto text-muted/60 mb-3" />
                <p className="text-muted font-medium">No match results available.</p>
              </Card>
            ) : (
              <div className="space-y-4">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center justify-between gap-4">
                              <div className="flex items-center gap-2.5 min-w-0">
                                <div className="h-6 w-6 rounded bg-primary/10 border border-primary/20 flex items-center justify-center font-bold text-xs text-primary flex-shrink-0">H</div>
                                <span className={`text-sm sm:text-base truncate ${isHomeWinner ? "font-bold text-text" : "font-medium text-text/75"}`}>
                                  {game.homeClubName} {game.homeTeamName}
                                </span>
                              </div>
                              <span className={`text-base font-extrabold px-2.5 py-0.5 rounded-md ${isHomeWinner ? "bg-primary text-white" : "bg-background text-muted"}`}>
                            ...
                            <div className="flex items-center justify-between gap-4">
                              <div className="flex items-center gap-2.5 min-w-0">
                                <div className="h-6 w-6 rounded bg-accent/10 border border-accent/20 flex items-center justify-center font-bold text-xs text-accent flex-shrink-0">A</div>
                                <span className={`text-sm sm:text-base truncate ${isAwayWinner ? "font-bold text-text" : "font-medium text-text/75"}`}>
                                  {game.awayClubName} {game.awayTeamName}
                                </span>
                              </div>
                              <span className={`text-base font-extrabold px-2.5 py-0.5 rounded-md ${isAwayWinner ? "bg-primary text-white" : "bg-background text-muted"}`}>
        ...
        <div className="mt-12 text-center border-t border-border/60 pt-8">
          <Card variant="outlined" padding="lg" className="max-w-2xl mx-auto bg-surface/30">
            <h4 className="font-bold text-text text-base sm:text-lg mb-2">Are you a Coach, Manager, or Admin?</h4>
            <p className="text-sm text-muted mb-4 max-w-lg mx-auto">
              Log in to report scores, update event schedules, manage player rosters, and access admin tools.
            </p>
            <div className="flex justify-center">
              <Link href="/login">
                <Button variant="primary" className="flex flex-row items-center gap-2 text-sm px-5 py-2.5">
